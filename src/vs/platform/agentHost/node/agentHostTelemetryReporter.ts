...
export class AgentHostTelemetryReporter {
	 ...
	 * @param model The model that produced the message, if known.
	 */
	modelMessageText(session: string, content: string, turnId: string, serviceRequestId: string | undefined, model: string | undefined): void {
		const restricted = this._restricted;
		if (!restricted || !content) {
			return;
		}
