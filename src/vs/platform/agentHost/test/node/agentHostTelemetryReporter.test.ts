...
suite('AgentHostTelemetryReporter', () => {
	...
	test('modelMessageText emits conversation.messageText (source=model) with headerRequestId + model, and no-ops on empty content', () => {
		const service = new TestRestrictedTelemetryService();
		const reporter = new AgentHostTelemetryReporter(service);

		reporter.modelMessageText(session, '', '3', 'svc-1', 'gpt-x'); // dropped: no content
		reporter.modelMessageText(session, 'sure, here you go', '3', 'svc-1', 'gpt-x'); // emitted
